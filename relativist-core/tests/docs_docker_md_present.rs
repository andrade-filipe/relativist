//! IT-0609-04 — `docs/operations/docker.md` exists and documents the hybrid coordinator.
//!
...
//! every `cargo test` invocation and catches a missing or stub `docs/operations/docker.md`
...
///   1. File exists at `docs/operations/docker.md`.
...
fn docker_md_exists_and_documents_hybrid_coordinator() {
    // Doc relocated in the 2026-06 docs regeneration:
    // docs/DOCKER.md -> docs/operations/docker.md.
    let docker_md = repo_root()
        .join("docs")
        .join("operations")
        .join("docker.md");
    ...
    assert!(
        docker_md.exists(),
        "IT-0609-04: docs/operations/docker.md must exist at {docker_md:?}"
    );

    let content = fs::read_to_string(&docker_md)
        .unwrap_or_else(|e| panic!("IT-0609-04: failed to read docs/operations/docker.md: {e}"));

    let lines = content.lines().count();
    assert!(
        lines >= 30,
        "IT-0609-04: docs/operations/docker.md must have >= 30 lines (got {lines})"
    );

    let lower = content.to_lowercase();

    assert!(
        lower.contains("hybrid coordinator") || lower.contains("hybrid-coordinator"),
        "IT-0609-04: docker.md must contain 'hybrid coordinator' \
    ...
    assert!(
        has_d006_ref,
        "IT-0609-04: docs/operations/docker.md must reference 'D-006', 'post-D-006', \
         or 'coordinator reduces a local partition'"
    );

    assert!(
        content.contains("bench-tcp"),
        "IT-0609-04: docs/operations/docker.md must contain 'bench-tcp' \
         (documents the new compose profile)"
    );

    assert!(
        content.contains("CHUNK_SIZE"),
        "IT-0609-04: docs/operations/docker.md must document the CHUNK_SIZE env var"
    );

    assert!(
        content.contains("MAX_PENDING_LIFETIME"),
        "IT-0609-04: docs/operations/docker.md must document the MAX_PENDING_LIFETIME env var"
    );

    assert!(
        content.contains("RECYCLE_POLICY"),
        "IT-0609-04: docs/operations/docker.md must document the RECYCLE_POLICY env var"
    );
    ...
    assert!(
        has_troubleshoot,
        "IT-0609-04: docker.md must contain a troubleshooting / pitfalls / \

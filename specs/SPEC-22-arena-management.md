...
**R22a (D-011 Amendment 2026-05-04 — clarification).** The M5 pathology that originally motivated R22 (recycled-id fragmentation under delta-mode operation without compaction — see SC-009 closure rationale and §3.8 A7) MUST still be detected by the corrected `effective_arena_size` metric of R22. Under M5 fragmentation, although `live_count` may remain modest (e.g., 10M live agents per partition), the `max_live_id` in `worker_agents` grows because consumed-then-recycled IDs leave high-numbered slots occupied while low-numbered slots churn through `None`/`Some` cycles — producing `max_live_id ≫ live_count`. Concretely: with `live_count = 10M` and `max_live_id = 100M`, `effective_arena_size = 100_000_001 > 4 × 10_000_000 = 40_000_000`, so R22's threshold fires and the sparse path is selected, preventing the 800 MB `vec![None; 100M]` allocation under the dense path. The boundary-case convention `worker_agents.is_empty() ⇒ effective_arena_size = 0 ⇒ threshold not exceeded ⇒ dense path returns `Net::new()`` (per `partition/helpers.rs:297-299`) is preserved unchanged. **(MUST)**

This closes the residual concern that the metric correction in R22 might lose the M5-pathology guard; under the corrected metric, the guard is preserved precisely because the original M5 evidence base (HVM2 / SC-009) is fundamentally about `max_live_id`, not about `id_range.end`. The pre-D-011 metric was an over-approximation that happened to subsume the M5 case while ALSO mis-routing healthy workloads; the post-D-011 metric is exact.
...
- Define `effective_arena_size := max_live_id + 1`, where `max_live_id := worker_agents.iter().copied().max()` (cast to `u64`). When `worker_agents` is empty, `effective_arena_size := 0` (see R22a-bis below for the empty-partition convention). This metric matches the actual `Vec<Option<Agent>>` size that the dense `build_subnet` allocates at `relativist-core/src/partition/helpers.rs:301-303` (`agents_len = max_id + 1`); it is decoupled from the planning `id_range` produced by `compute_id_ranges` (`base_next_id × 10` chunk multiplier).

...
**Status:** Reviewed v2.4 — D-011 Round 2 amendment 2026-05-04: closes spec-critic Round 1 findings SC-022 (R22 "MAY" loophole tightened — SPARSE forbidden below threshold), SC-023 (R22 build-time-only scope made explicit), SC-024 (R22a M5-detection mechanism re-derived via streaming/repartition path; forwarding reference to SC-009 closure log added), SC-025 (R30 SHOULD-rename → MUST-rename tied to TASK-0612), SC-026 (R22 `live_agent_count := worker_agents.len()` defined inline), SC-028 (change-log shorthand tightened to reference `id_range.end - id_range.start`). SC-027 REJECTED with rationale (pre-existing, orthogonal to D-011 metric correction). Earlier: v2.3 — D-011 Amendment 2026-05-04: R22 + R30 metric corrected from `id_range > 4 × live_agent_count` to `effective_arena_size > 4 × live_agent_count` (where `effective_arena_size = max_live_id + 1`, matching dense `build_subnet` allocation); R22a added to confirm M5 pathology is still detected; closes BLOCKER 2026-05-04 (partition perf regression). v2.2 — R10b/R10c trigger broadened per SPEC-21 §3.8 A6 (`delta_mode` → `(delta_mode || streaming_active)`); §3.8 A11 added per D-011 Phase A (SPEC-19 R35a — `CompactSubnet` wire encoding of `free_list`; closes QA-D009-001)
...
**R22a (D-011 Amendment 2026-05-04 — clarification, refined v2.4 per SC-024).** The M5 pathology that originally motivated R22 (recycled-id fragmentation under delta-mode operation without compaction — see SC-009 closure rationale in `docs/spec-reviews/SPEC-REVIEW-22-round-2-2026-04-25.md` and §3.8 A7) MUST still be detected by the corrected `effective_arena_size` metric of R22. The mechanism by which `max_live_id ≫ live_count` arises under M5 conditions is as follows — explicit derivation (closes SC-024):

> **Fragmentation mechanism.** Under streaming or repartition, the `split_with_config` call that produces `worker_agents[i]` for partition `i` draws agent IDs from a `sigma` whose domain is the previous round's live set. During the previous round's local reduction, CON-DUP commutation fires repeatedly: each fire consumes 2 agents and creates 4 (SPEC-01 T5; §4.7 table). When the worker's free-list is empty (e.g., expansion-dominated phase or under `RecyclePolicy::DisableUnderDelta` while `delta_mode || streaming_active`), the 4 created agents are allocated from `next_id` upward, monotonically. After many such fires, `next_id` advances far past any low-numbered ID still held by the surviving live set. When the next `split_with_config` call assigns a subset of those high-numbered live agents to partition `i` (e.g., FENNEL preserves locality but admits non-contiguous ID assignment, or a streaming-finalize repartition draws live IDs whose values were drawn from the post-fragmentation `next_id`), `worker_agents[i]` contains IDs whose `max_live_id` is `~next_id_after_fragmentation`, while `worker_agents[i].len()` (= `live_count`) reflects only the partition's share of the surviving live set. The pattern is one-way drift of `next_id`, not LIFO churn of low-numbered slots — the LIFO free-list of §4.7 reuses low-numbered slots first (most-recently-freed = lowest under the access pattern that interleaves create/remove), so fragmentation is driven by the creation-heavy phase that bumps `next_id` past any reuse rate, not by low-slot churn.

> **Worked example.** Worker 0 enters round N with `live_count = 100K` and `next_id = 100K`. Round N is CON-DUP-dominated; the free-list is disabled (`streaming_active = true`, `RecyclePolicy::DisableUnderDelta`). After 25M CON-DUP fires (50M agents created, 50M consumed across all workers; per worker ~10M survive → `live_count = 10M`), `next_id` of worker 0 advances to ~100M. The streaming-pipeline's next finalize calls `to_dense` for partition 0, producing `worker_agents = [some IDs in [0, 100K) ∪ some IDs in [100K, 100M)]` with `max_live_id ≈ 99_999_999` and `worker_agents.len() = 10_000_000`. Then `effective_arena_size = 100_000_000 > 4 × 10_000_000 = 40_000_000`; R22's threshold fires and the SPARSE path is taken, preventing the dense `vec![None; 100M]` (~800 MB) allocation that would otherwise consume the 2 GB coordinator budget.

The boundary-case convention `worker_agents.is_empty() ⇒ effective_arena_size = 0 ⇒ threshold not exceeded ⇒ dense path returns `Net::new()`` (per `partition/helpers.rs:297-299`) is preserved unchanged. **(MUST)**

**Forwarding reference (per SC-024 Option (b)).** The original SC-009 evidence base (HVM2 / AC-006 — "200K agents at 800 MiB") is documented in `docs/spec-reviews/SPEC-REVIEW-22-round-2-2026-04-25.md` SC-009 closure. The HVM2 pathology was driven by `max_live_id`-style fragmentation — high-numbered ID assignments that the dense path materializes regardless of how dense the live set is in the low-numbered region. The corrected `effective_arena_size := max_live_id + 1` metric covers the same evidence base: the pre-D-011 `id_range.end - id_range.start` metric was an over-approximation that happened to subsume the M5 case (because under `ContiguousIdStrategy` the planning range is itself bounded by `next_id × 10`, which tracks `max_live_id` up to a constant factor) while ALSO mis-routing healthy workloads where `id_range.end` is allocated as `max(100_000, base_next_id × 10)` per partition. The post-D-011 metric is exact: it measures the actual arena the dense path would allocate, not the planning-range upper bound.

R22a's claim is now traceable: (i) the fragmentation mechanism is explicit (one-way `next_id` drift under CON-DUP-dominated phases with free-list disabled), (ii) the `worker_agents` indirection from `split_with_config` is named (the previous round's `next_id` reaches the next round's `max_live_id` only when the partitioner re-assigns those high-numbered live agents to a worker, which is the streaming-finalize / repartition pattern, NOT a single round's local reduction — consistent with R22's "build-time only" scope above), and (iii) the SC-009 evidence-base coverage is asserted with a forwarding reference for audit.

**Bench-harness measurement protocol:** see SPEC-09 §3.3.X (R18c–R18g) and §4.9 for the `BenchmarkResult` provenance fields (`representation`, `chunk_size`, `recycle_policy`) that record the path taken on each datapoint, and for the SparseNet micro-bench acceptance gate (D-011 Phase D, `dual_tree` only: median R18a under Sparse must be < 80% of Dense at the same `input_size`).
...
- Define `effective_arena_size := max_live_id + 1`, where `max_live_id := worker_agents.iter().copied().max()` (cast to `u64`). When `worker_agents` is empty, `effective_arena_size := 0` (see R22a below for the empty-partition convention). This metric matches the actual `Vec<Option<Agent>>` size that the dense `build_subnet` allocates at `relativist-core/src/partition/helpers.rs:301-303` (`agents_len = max_id + 1`); it is decoupled from the planning `id_range` produced by `compute_id_ranges` (`base_next_id × 10` chunk multiplier).
...
- If `effective_arena_size > 4 * partition.live_agent_count`, `build_subnet` MUST use `SparseNet` and then call `to_dense(Some(partition.id_range.clone()))` before the reduction loop. This is the M5-target case (SC-009) and the M5 ID-fragmentation pathology (R22a).
- Otherwise (i.e., `effective_arena_size <= 4 × partition.live_agent_count`), `build_subnet` MUST use the dense path. The SPARSE path is FORBIDDEN below threshold, regardless of `PartitionConfig.sparse_build` (R30). The `sparse_build = false` flag is honored only as a rejection signal: when `sparse_build = false` AND threshold IS exceeded, `build_subnet` MUST return `PartitionError::DenseAllocationExceedsThreshold` per R30. This closes SC-022 — without this normative bound the spec letter would still permit re-introducing the +63% partition+merge regression observed in BLOCKER 2026-05-04 by choosing the SPARSE path on healthy workloads under the `sparse_build = true` default. The implementation in `relativist-core/src/partition/helpers.rs:447-475` already enforces this binding; SC-022 closure aligns the spec letter with the implementation.
...
The `4×` constant is unchanged from the pre-D-011 formulation; only the metric the constant multiplies has been corrected. Empirical evidence for the metric correction is recorded in `docs/next-steps.md` BLOCKER 2026-05-04: the previous formulation `id_range > 4 × live_agent_count` measured the planning range (`id_range.end - id_range.start`, where `id_range` is allocated per-worker by `compute_id_ranges` as `max(100_000, base_next_id × 10)`), which is decoupled from the actual `Vec<Option<Agent>>` size the dense path allocates. Under `ContiguousIdStrategy` for healthy workloads (live agents densely packed), the planning range is 5–800× the live count, routing every partition through the SPARSE branch and producing a +83% wall-clock regression on `ep_annihilation_con 5M w=2` (12 s → 22 s) — proven by 7-point bisect plus probe instrumentation. Replacing `id_range_size` with `effective_arena_size` aligns the threshold with the dense path's true memory cost.

**Determinism note (T7).** `worker_agents.iter().copied().max()` is order-independent on `&[AgentId]` (= `&[u32]`); for any fixed input slice the result is deterministic. The slice itself is constructed deterministically by `split_with_config` from `sigma`; therefore the `effective_arena_size` is deterministic and SPEC-01 T7 (Determinism of Reduction → IsoNF) is preserved.

Under the current `ContiguousIdStrategy`, the last worker's dense subnet allocates `vec![None; max_id + 1]` even when it owns a fraction of the agents. The MUST upgrade above (was SHOULD in v1) closes SC-009: at M5 scale (100M agents on 2 GB coordinator), a worker holding 10M live agents whose `max_live_id` is 100M (heavy ID fragmentation under delta-mode recycling without compaction) would allocate 800 MB of `vec![None; 100M]` under the dense path — pathological. The 4× threshold against the corrected metric provides a clean safety margin. **(MUST)**
...
**R30.** The hybrid approach (R22: `SparseNet` for `build_subnet`, `Net` for reduction) MUST be configurable via a `sparse_build: bool` flag in `PartitionConfig` or equivalent. Default: `true` (use sparse construction). Setting `false` reverts to the current dense construction for backward compatibility or when the overhead of conversion is undesirable. Note that under R22's threshold rule (`effective_arena_size > 4 × live_agent_count`, where `effective_arena_size = max_live_id + 1` per R22), `sparse_build = false` MUST be rejected at the top of `build_subnet` with a clear `PartitionError::DenseAllocationExceedsThreshold` because the dense path would inflate beyond M5 budget. The flag's `false` setting is therefore only honored when the threshold is NOT exceeded. The `PartitionError::DenseAllocationExceedsThreshold` payload MUST report `effective_arena_size` (renaming the pre-D-011 `id_range_size` field) alongside `live_count` and `partition_index`, so that diagnostics directly identify the actual `Vec<Option<Agent>>` size that would have been allocated. The rename is normative; implementations MUST land the field rename atomically with the threshold metric change. TASK-0612 of the D-011 partition-perf-fix bundle is the canonical implementation hand-off (see `docs/plans/2026-05-04-d011-partition-perf-fix.md` §5 Task 4). Closure log §4 records the spec-ahead-of-code window between this v2.4 amendment landing and TASK-0612 completion: R30's MUST is calibrated against TASK-0612's atomic landing, not against the in-tree code at the moment v2.4 lands. This closes SC-025. **(MUST)**
...
**A11. SPEC-19 §3.4 R35a — `CompactSubnet` wire encoding of `free_list` (D-011 Phase A; closes QA-D009-001).**
- *Old text (SPEC-19 §3.4 wire-protocol catalog, pre-R35a):* `CompactSubnet` (`relativist-core/src/partition/compact.rs:38`) is named in R35 as the wire form of `Partition.subnet` for `InitialPartition` / `FinalStateResult` payloads, but the encoding of `Net.free_list` through that adapter is unspecified. The live implementation drops `free_list` on `from_net()` and reconstructs it as `Vec::new()` on `into_net()`.
- *New text (SPEC-19 R35a clauses (a)-(g)):* `CompactSubnet` MUST encode `free_list: Vec<AgentId>` as a new struct field positioned after `root`, with `from_net()` capturing `net.free_list.clone()` and `into_net()` materializing the field verbatim. Round-trip `Net -> CompactSubnet -> Net` MUST preserve `free_list` byte-for-byte; the `nets_equivalent` test helper at `compact.rs:152-158` MUST be extended to compare `free_list`. Both empty and non-empty `free_list` boundary cases MUST be covered by the acceptance suite. The `--features zero-copy` rkyv path MUST be extended with at least one populated-free-list round-trip test mirroring `rkyv_round_trip_compact_subnet_minimal` at `compact.rs:301-334`.
- *Rationale:* SPEC-19 R35a MUST be implemented atomically with this spec's free-list across the wire path. Without R35a, R10a/R10b/R10c degrade silently on multi-worker TCP transport: the coordinator's `build_subnet` (R10a) populates a per-partition free-list, the wire silently empties it, and the worker's subsequent `create_agent` calls fall back to `next_id` allocation regardless of available recycle slots — reproducing SPEC-02 R10's pre-SPEC-22 monotonic-arena pathology selectively on the distributed path while in-process tests pass. Furthermore, R12 reconciliation across `merge` becomes unsound on the network: the coordinator-side `merge` walks per-partition free-lists that arrived empty, producing a merged `Net` whose `next_id` is correct but whose ID-reuse is permanently lost. Bench Phase E-4 (`docker compose run bench-tcp --benchmark ep_annihilation --workers 2 --chunk-size 100 --mode tcp`) is the integration-level acceptance gate. **PROTOCOL_VERSION bump: from `PREVIOUS_LIVE_VERSION` to `PREVIOUS_LIVE_VERSION + 1`** (live constant currently `6` post D-009 + D-010 at `protocol/coordinator.rs:197`; the R35a bump lands at `7`, but defensive language is mandated in line with SPEC-06 R3b / SPEC-19 R37 / SPEC-22 R9a precedents). Cross-references: SPEC-04 §A7 wire-round-trip clause; SPEC-18 R28 (PROTOCOL_VERSION constant) and R33 (round-trip correctness extension to `free_list`).
...
### D-011 Amendment — 2026-05-04 — `effective_arena_size` threshold metric (closes BLOCKER 2026-05-04)

**Motivation.** `docs/next-steps.md` BLOCKER 2026-05-04 documents a +83% wall-clock regression on `ep_annihilation_con 5M w=2` (12 s → 22 s) that was bisected to commit `d6411be` (D-009 Phase C, SPARSE branch introduction). Probe instrumentation in `partition::helpers::build_subnet_with_config` confirmed that the pre-D-011 metric `id_range > 4 × live_agent_count` was measuring the size of the planning range produced by `compute_id_ranges` — specifically `id_range.end - id_range.start`, which under SPEC-04 R18's `ContiguousIdStrategy` evaluates to `max(100_000, base_next_id × 10)` per partition (the `base_next_id × 10` chunk multiplier is the formula `compute_id_ranges` uses to derive the range size, not the metric itself; closes SC-028 — change-log shorthand tightened in v2.4). The pre-D-011 metric was NOT measuring the actual `Vec<Option<Agent>>` size that the dense `build_subnet` would allocate (`max_live_id + 1`, per `helpers.rs:301-303`). For healthy workloads under `ContiguousIdStrategy`, the planning range is 5–800× the live count; the OLD metric routed every healthy partition through the SPARSE (HashMap-then-`to_dense`) branch, producing the regression. The NEW metric `effective_arena_size > 4 × live_agent_count` matches the dense allocation behavior exactly.

**Scope.**

| ID | Action | Summary |
|----|--------|---------|
| R22 | Modified | Metric replaced; `effective_arena_size := max_live_id + 1` defined and tied to `helpers.rs:301-303`. Determinism-T7 note added (`max()` is order-independent). The `4 ×` constant is unchanged. |
| R30 | Modified | Rejection rule re-worded against the new metric. Error variant `PartitionError::DenseAllocationExceedsThreshold` SHOULD report `effective_arena_size` (rename of `id_range_size` field). |
| R22a | Added | Clarification that M5 pathology (recycled-id fragmentation under delta mode) is still detected by the corrected metric because it manifests as `max_live_id ≫ live_count`. Empty-partition convention preserved (`worker_agents.is_empty() ⇒ effective_arena_size = 0 ⇒ dense path returns `Net::new()``). |

**Cross-references audited (best-effort grep `id_range.*4.*live\|id_range_size.*4` over `specs/`):**
- SPEC-04 §4.5.1 Amendment A7 — cites the formula; updated to point to R22 with the corrected metric.
- SPEC-21 §3.3 (line 752 `id_range > 4 * live_agent_count` — accumulator threshold); §4.9 finalize-time threshold contract (line 852); inline doc-comments in PartitionAccumulator (lines 764, 782) — updated to use `effective_arena_size` and forward to SPEC-22 R22.

**Out of scope:** `compute_id_ranges` chunk multiplier (`base_next_id × 10`) is preserved unchanged — that is a separate amendment surface that is not required to fix the regression once the metric is corrected (Option D in `docs/plans/2026-05-04-d011-partition-perf-fix.md` §2). R23 (CI lint forbidding `SparseNet` in `reduction/`) is orthogonal. The wire format / `PROTOCOL_VERSION` is unaffected — this is a build-time threshold decision, not a wire field. SparseNet itself (§4.4, §4.6 `to_dense`) is unchanged; the SPARSE path remains, just taken less often.

**Implementation hand-off.** The implementation lives in TASK-0611..0614 of the D-011 partition-perf-fix bundle (`docs/plans/2026-05-04-d011-partition-perf-fix.md` §5):
- `relativist-core/src/error.rs` — rename `id_range_size` field of `DenseAllocationExceedsThreshold` to `effective_arena_size`; update `#[error(...)]` format string.
- `relativist-core/src/partition/helpers.rs:432-444` — replace `id_range_size` computation with `effective_arena_size = worker_agents.iter().copied().max().map(|m| m as u64 + 1).unwrap_or(0)`; update doc-comment block at `helpers.rs:402-420`.
- `relativist-core/src/partition/helpers.rs` (test mod, lines 1340–1545) — rewrite UT-0484-02..05 + UT-0492-01..02 to use scattered IDs that exercise the NEW metric.
- `relativist-core/tests/d011_partition_perf_witness.rs` — new integration regression witness (TASK-0613).

**Test floor impact (from spec amendment alone):** none. The implementation tasks 0612-0613 will each modify the in-file test count (no net delta if the rewrites are 1:1, plus +1 for the new integration witness in `tests/`). Floors entering D-011 partition-perf-fix: 1683 default / 1726 zero-copy / 1680 streaming-no-recycle. v1 floor: 690.

**Files cross-referenced for the amendment audit:** `codigo/relativist/relativist-core/src/partition/helpers.rs:280-477`, `codigo/relativist/specs/SPEC-04-partition.md:407-412`, `codigo/relativist/specs/SPEC-21-streaming-generation.md:740-854`, `codigo/relativist/docs/next-steps.md` BLOCKER 2026-05-04 block (lines 9-95), `codigo/relativist/docs/plans/2026-05-04-d011-partition-perf-fix.md` §1, §3, §8.

**Spec-critic Round 1:** REQUESTED (small surface, but touches core threshold semantics; an adversarial second look is appropriate). Verdict expected: `CONDITIONAL_PASS` (this is a corrective amendment, not a redesign). Pre-empted attack vectors are documented in `docs/plans/2026-05-04-d011-partition-perf-fix.md` §8 (Q1 — `4×` constant unchanged; Q3 — empty partition; Q4 — single agent at `id = u32::MAX-1`; Q6 — T7 determinism).

**Closure log:** `codigo/relativist/docs/spec-reviews/SPEC-22-amendment-2026-05-04-d011-blocker.md`.

**Status transition:** Reviewed v2.2 → Reviewed v2.3 (D-011 Amendment 2026-05-04 applied; no Round 2 re-open).

**Stage gate:** This closure log lands BEFORE Stage 1 (TASK-SPLITTER) and Stage 2 (TEST-GENERATOR) per the SDD pipeline contract.

### D-011 Round 2 Amendment — 2026-05-04 — closes Round 1 spec-critic findings (SC-022..SC-028)

**Context.** Spec-critic Round 1 review (`docs/spec-reviews/SPEC-22-round1-critic-2026-05-04-amendment.md`) issued verdict `NEEDS_REVISION` on the v2.3 D-011 Amendment (commit `e941273`). Seven findings were raised: 1 HIGH (SC-022), 3 MEDIUM (SC-023, SC-024, SC-025), 3 LOW (SC-026, SC-027, SC-028). Six are accepted and closed in v2.4; one (SC-027) is rejected with rationale.

**Scope.**

| ID | Severity | Decision | Action in v2.4 |
|----|----------|----------|----------------|
| SC-022 | HIGH | ACCEPT | R22 third bullet rewritten: "MUST use the dense path. The SPARSE path is FORBIDDEN below threshold, regardless of `PartitionConfig.sparse_build`." Spec letter now matches `helpers.rs:447-475` enforcement; the +63% partition+merge re-regression vector is closed. |
| SC-023 | MEDIUM | ACCEPT | R22 extended with explicit "Build-time only" paragraph: the metric is evaluated EXACTLY ONCE at `build_subnet` entry; in-round arena growth is governed by SPEC-02 R11 + SPEC-22 R3/R4, not by R22. M5-detection guard fires at the next `build_subnet` entry (streaming/repartition), not within a single round. |
| SC-024 | MEDIUM | ACCEPT | R22a re-derived: explicit fragmentation mechanism (one-way `next_id` drift under CON-DUP-dominated phases with free-list disabled), explicit `worker_agents` indirection through `split_with_config`, forwarding reference to `docs/spec-reviews/SPEC-REVIEW-22-round-2-2026-04-25.md` SC-009 closure for the HVM2 evidence base. The "low-numbered slots churn" mental model is replaced with the correct one-way drift mechanism. |
| SC-025 | MEDIUM | ACCEPT | R30 SHOULD-rename → MUST-rename, tied explicitly to TASK-0612's atomic landing. Spec-ahead-of-code window acknowledged in the v2.3 closure log §4. |
| SC-026 | LOW | ACCEPT | R22 second bullet adds inline definition `partition.live_agent_count := worker_agents.len()` with cross-reference to SPEC-04 R15a / `helpers.rs:316-321`. |
| SC-027 | LOW | REJECT-WITH-RATIONALE | The `min(id_range.end, arena_len)` clamp at `helpers.rs:351-353` is pre-existing (predates D-011) and orthogonal to the metric correction (the critic acknowledges this in finding text: "this is pre-existing and orthogonal to the D-011 metric correction"). The clamp is not introduced by D-011, only exercised more frequently because the corrected SPARSE path now runs on a different workload distribution. Surfacing it in this corrective amendment would expand scope beyond the user-stated constraint ("Keep changes minimal — surgical wording fixes, not a rewrite"). The clamp's normative status is a separate hardening task to be filed against R10a outside the D-011 bundle. |
| SC-028 | LOW | ACCEPT | Change-log line in §11 D-011 Amendment 2026-05-04 entry (line 906 of v2.3) tightened: "the size of the planning range produced by `compute_id_ranges` — specifically `id_range.end - id_range.start`, which under SPEC-04 R18's `ContiguousIdStrategy` evaluates to `max(100_000, base_next_id × 10)` per partition". |

**Cross-references audited (no propagation required).** SPEC-04 §4.5.1 A7 already cites "the canonical metric is normative there [SPEC-22 R22]" and forwards to R22 for the actual formula — no edit needed in SPEC-04 since the v2.4 R22 changes are consumed by reference. SPEC-21 §3.3 / §4.9 / §8 Q6 (5 spots) likewise forward to SPEC-22 R22 by reference for the threshold rule; the v2.4 wording changes (build-time-only scope, FORBIDDEN-below-threshold) propagate transitively without textual edit. Verified: the SPARSE-below-threshold-FORBIDDEN clause (SC-022) does not contradict SPEC-21's accumulator design because the streaming pipeline's accumulator selection (SPEC-21 §4.9) is a CONSTRUCTION-time choice that finalizes via `to_dense` at the SAME R22 threshold gate; if the threshold is not exceeded at finalize, the dense `Net` materializes via the dense path per SC-022's tightened R22, consistent with SPEC-21's "selected at construction time" rule.

**Out of scope for v2.4.** SC-027 (clamp at `helpers.rs:351-353`) is rejected per above. Rust code in `relativist-core/src/` is unchanged by v2.4 (this is a spec-only amendment closing Round 1 findings). TASK-0612 (the field rename + metric implementation) remains the canonical implementation hand-off. Bench-suite changes are unaffected.

**Test floor impact (from v2.4 alone):** none. v2.4 is a normative-only refinement of v2.3; no spec-derived test additions or removals fire from this commit. Floors unchanged: 1683 default / 1726 zero-copy / 1680 streaming-no-recycle. v1 floor: 690.

**Closure log:** `docs/spec-reviews/SPEC-22-round2-defense-2026-05-04-amendment.md` (Round 2 defense, this dispatch).

**Status transition:** Reviewed v2.3 → Reviewed v2.4 (D-011 Round 1 spec-critic findings closed). v2.4 is final for the D-011 corrective amendment; no Round 3 expected absent new evidence.

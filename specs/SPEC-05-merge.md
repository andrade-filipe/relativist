...
**Review history:** Round 1 critic (SPEC-05-round1-critic.md, 14 issues: 2 CRITICAL, 3 HIGH, 4 MEDIUM, 5 LOW), Round 2 defender response (SPEC-05-round2-defender.md). v3.1 (2026-04-10): added `strict_bsp` mode to `GridConfig` and `run_grid` so the BSP loop can iterate for more than one round when cross-partition cascades exist; lenient mode (default) preserves the v3 behavior with zero regression. Source: plano curious-sleeping-patterson, Fase 1.
...
**R15.** After the merge, the coordinator MUST reduce all border redexes (and any new redexes generated by those reductions) by invoking `reduce_all` (SPEC-03) on the merged net, **unless `GridConfig.strict_bsp = true` (R30a), in which case border resolution is deferred to the next BSP round**. This lenient-mode approach is strictly more complete than the Haskell prototype's selective resolution (`foldl reduce merged borderPairs`, AC-004), which may leave non-border redexes generated by CON-DUP cascades for the next round. Strict mode deliberately relies on this deferral to expose cross-partition cascades as additional BSP rounds for experimental measurement (R30a, SPEC-09 R18). **(MUST for lenient mode; deferred to next round under strict mode per R30a)**
...
**R30a.** The grid loop MAY run in strict BSP mode via `GridConfig.strict_bsp = true`. In strict mode, the border resolution phase does not call `reduce_all` on the merged net; instead it relies on `Net::connect` auto-enqueueing border redexes into `merged_net.redex_queue` (SPEC-02, R13), which are then processed by workers in subsequent rounds until normal form is reached. When `GridConfig.strict_bsp = false` (the default, **lenient mode**), the coordinator invokes `reduce_all` on the merged net as specified in R15, collapsing all border-origin cascades into the current round. Both modes preserve G1 (SPEC-01); they differ only in how reduction work is distributed across BSP rounds. **(MAY)**

- **Lenient mode (default, R15-R18):** After merge, the coordinator runs `reduce_all` on the merged net. All border redexes and their CON-DUP cascades are resolved at the coordinator in a single pass. As a consequence, `GridMetrics.rounds` is always `<= 1` for terminating nets (in practice, exactly 1 when the initial net is not already in Normal Form). This is an optimization that avoids a subsequent split/merge for the cascade work.
- **Strict mode (opt-in):** After merge, the coordinator skips `reduce_all` entirely. Border redexes inserted by `Net::connect` during merge (R5, R12) remain in the merged net's redex queue. If the queue is non-empty at the termination check, a new round is initiated: the merged net is re-partitioned (R28), workers reduce the border-origin redexes locally, and any further cross-partition cascades become new border redexes in the next merge. `GridMetrics.rounds` therefore reflects the actual number of BSP iterations driven by cross-partition cascades; for terminating nets, `rounds >= 1` and is bounded above by the total number of interactions (SPEC-01, T7).

In both modes, the fundamental correctness argument (Section 4.6; ARG-001; ARG-003) is identical. Strong confluence (SPEC-01, T4) guarantees that the same Normal Form is reached regardless of how many rounds the border work is spread across. Strict mode is the primary execution mode for experimental measurements that need to exercise the multi-round BSP loop (e.g., the `cascade_cross` benchmark in SPEC-09 R18, and the per-round RTT cost that the Phase 3 LAN campaign will measure). Lenient mode is the production default because it is strictly faster for small/medium nets where the cost of one extra round exceeds the cost of one coordinator-side `reduce_all`.

**Backwards compatibility:** Because `strict_bsp` defaults to `false`, no existing caller needs modification and no test of `run_grid` under lenient semantics changes behavior. The distinction is observable only through `GridMetrics.rounds` and `GridMetrics.border_reduce_time_per_round` (which is ~0 in strict mode by construction of R30a).
...
pub struct GridConfig {
    ...
    /// BSP mode selector (R30a).
    ///
    /// - `false` (default, **lenient**): the coordinator runs `reduce_all`
    ///   on the merged net after each merge, collapsing all border-origin
    ///   cascades into the current round. `GridMetrics.rounds <= 1` for
    ///   terminating nets.
    /// - `true` (**strict**): the coordinator skips the post-merge
    ///   `reduce_all`. Border redexes inserted by `Net::connect` during
    ///   merge (SPEC-02 R13) remain in the merged queue and are processed
    ///   by workers in subsequent rounds, giving a faithful BSP iteration
    ///   count for experimental analysis of cross-partition cascades
    ///   (SPEC-09 R18).
    ///
    /// See SPEC-05 Section 3.5 R30a and Section 4.5 for the behavioural
    /// difference, and Section 3.5 for the backwards-compatibility guarantee.
    pub strict_bsp: bool,
...
impl Default for GridConfig {
    fn default() -> Self {
        Self {
            num_workers: 1,
            max_rounds: None,
            // Default to lenient mode to preserve the v3 behavior exactly
            // (R30a backwards-compatibility guarantee).
            strict_bsp: false,
        }
    }
}
...
fn run_grid(net, config, strategy) -> (Net, GridMetrics):
    ...
    loop:
        ...
        // === PHASE 4: RESOLVE BORDERS ===
        // Lenient vs Strict BSP branch (R30a).
        //
        // Lenient (default): exhaust the border-origin redexes at the
        // coordinator in a single reduce_all pass. This collapses all
        // cross-partition cascades into the current round, so metrics.rounds
        // converges to 1 for any terminating net.
        //
        // Strict: skip reduce_all entirely. Border redexes enqueued by
        // Net::connect during merge (SPEC-02 R13) stay in merged_net's
        // redex queue; the termination check below will see a non-empty
        // queue and trigger another round, where workers process those
        // redexes locally under a fresh partitioning. This yields a
        // faithful BSP iteration count bounded above by the total number
        // of interactions (SPEC-01, T7).
...
> **Note on lenient vs strict in the termination check:** The termination check `current_net.redex_queue.is_empty()` is unchanged between modes; what differs is the contents of the queue at the moment of the check. In lenient mode, the queue is emptied by the coordinator-side `reduce_all` at Phase 4, so the termination check almost always succeeds on the first round (for nets that were not already in Normal Form, `rounds == 1` afterwards). In strict mode, the queue still holds border redexes inserted by `Net::connect` during merge, so the termination check fails whenever cross-partition cascades exist, and the loop iterates. Both modes reach the same Normal Form (SPEC-01, T4 + T6) and G1 holds identically (SPEC-01, Section 3.4 note added in SPEC-01 v3.1).
...
### 4.5a Lenient vs Strict BSP Modes

This subsection formalizes the two modes introduced by R30a and consumed by the pseudocode of Section 4.5. Both modes implement the same grid cycle skeleton (split, local reduce, merge, terminate); they differ only in one conditional inside Phase 4.

**Definitions.** Let `mu` be a terminating net and let `run_grid_lenient(mu, n)` and `run_grid_strict(mu, n)` denote `run_grid(mu, config, strategy)` with `config.strict_bsp = false` and `config.strict_bsp = true` respectively, for some fixed `n` and partition strategy. Let `R_lenient(mu, n)` and `R_strict(mu, n)` be the resulting `GridMetrics.rounds`.

**Property 1 (Same normal form).** For every terminating `mu` and every `n >= 1`:

```
run_grid_lenient(mu, n).net  ~  run_grid_strict(mu, n).net  ~  reduce_all(mu)
```

where `~` denotes graph isomorphism (SPEC-00 Section 6.12). This is an immediate corollary of SPEC-01 T4 (strong confluence), T6 (uniqueness of normal form), and G1: both modes perform reductions that collectively reach a Normal Form, and strong confluence makes the order and distribution of reduction steps irrelevant for the final result.

**Property 2 (Round count ordering).** For every terminating `mu` and every `n >= 1`:

```
R_lenient(mu, n)  <=  R_strict(mu, n)
```

Proof sketch. In lenient mode, Phase 4 runs `reduce_all` on the merged net, so the termination check at the end of round 1 sees an empty queue whenever `mu` reduces to Normal Form in finitely many steps (SPEC-01 I5). Therefore `R_lenient(mu, n) <= 1`, with equality iff `mu` has at least one initial redex. In strict mode, Phase 4 is a no-op; the termination check at the end of round `k` sees an empty queue iff no border redex remained enqueued after merge, which can only happen once all cross-partition cascades have been fully propagated and resolved via local reduction in some later round. Therefore `R_strict(mu, n) >= R_lenient(mu, n)` always. Equality occurs when the initial partitioning places all active pairs inside single partitions (no border redexes and no cascade promotes an internal wire to a cross-partition wire), in which case Phase 4 has nothing to do in either mode.

**Property 3 (Upper bound on strict rounds).** For every terminating `mu` and every `n >= 1`:

```
R_strict(mu, n)  <=  S(mu)
```

where `S(mu)` is the total number of interactions to reach Normal Form (invariant by SPEC-01 T7). Proof sketch. Each strict round reduces at least one interaction (otherwise the merged queue would be empty and the loop would terminate at the end of the previous round). Therefore the number of strict rounds is bounded by the total interaction budget, which is finite for terminating nets.

**Property 4 (Total work is identical).** The sum of per-rule interaction counts across all rounds is identical in lenient and strict modes, up to the canonical partitioning:

```
sum(metrics_lenient.total_interactions_by_rule)
  ==  sum(metrics_strict.total_interactions_by_rule)
  ==  S(mu)
```

This is a direct corollary of SPEC-01 T7 (invariance of total interaction count) combined with Property 1. Strict mode does not add any extra interactions; it only redistributes them across more BSP rounds and from the coordinator's `reduce_all` budget into the workers' local budgets.

**Operational consequence for metrics.** In lenient mode, `GridMetrics.border_interactions_per_round` accumulates the full cascade budget at the coordinator, and `GridMetrics.border_reduce_time_per_round` measures the wall-clock of the coordinator-side `reduce_all`. In strict mode, `GridMetrics.border_interactions_per_round[k]` is always 0 (no coordinator-side reduction), and `GridMetrics.border_reduce_time_per_round[k]` is ~0 (only the `Instant::elapsed` overhead of the no-op branch). The interactions that would have appeared under `border_interactions_per_round` in lenient mode appear instead under `local_interactions_per_round` in subsequent rounds, because the workers in round `k+1` pick up the border-origin redexes that were left in the merged queue at round `k`.

**When to use which mode.**

| Criterion | Lenient (default) | Strict |
|-----------|-------------------|--------|
| Production execution of arithmetic nets (SPEC-14 ChurchAdd, ChurchMul) | Yes | No (unnecessary overhead) |
| Performance baseline for local benchmarks (SPEC-09 Profile A/B default measurements) | Yes | No |
| Multi-round BSP iteration count measurement (cascade_cross, DualTree as a stress test) | No | Yes (R30a, SPEC-09 R18) |
| RTT-per-round cost isolation for Phase 3 LAN campaign | No (collapses rounds) | Yes |
| G1 regression test against `reduce_all` | Both (Property 1) | Both |

Strict mode is NOT intended to replace lenient mode as the default; it is a diagnostic mode that makes multi-round BSP cost measurable without changing the correctness guarantees.
...
```
   ...
   |       |                                                   |
   |       v                                                   |
   | [5] RESOLVE BORDERS (R30a: lenient vs strict)             |
   |     if config.strict_bsp:                                 |
   |         skip reduce_all; border redexes remain in queue   |
   |         and are processed by workers in the next round    |
...
If the partitioning separated all Active Pairs, Phase 2 does no work (each partition is already in local Normal Form). Phase 3 detects all border redexes; Phase 4 resolves them under lenient mode (via coordinator-side `reduce_all`) or defers them under strict mode (R30a). Under lenient mode, if the resolution does not generate new redexes, the loop terminates in one round; if it generates new redexes (via CON-DUP), the coordinator's `reduce_all` keeps going until the merged queue is empty, still within the current round. Under strict mode, the loop iterates: round `k` merges, leaves border redexes in the queue, the termination check fails, round `k+1` re-partitions and workers reduce the border-origin redexes locally; cascades that cross the new partition boundaries become new border redexes, and so on until Normal Form.

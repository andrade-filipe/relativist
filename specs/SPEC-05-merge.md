...
**R1.** The `merge` function MUST accept as input a `Vec<Partition>` (partitions after local reduction) and a border map `HashMap<u32, (PortRef, PortRef)>` (from the `PartitionPlan`), and return a single recombined `Net`. **(MUST)**
...
**R9.** The redex queue of the result net MUST contain: (a) any internal redexes remaining from each partition's local reduction (excluding pairs that reference FreePort sentinels), (b) new redexes created by boundary reconnection (detected automatically by `Net::connect` per SPEC-02, R13). **(MUST)**
...
**R18.** The `reduce_all` after merge MUST process both border redexes and any residual local redexes that may exist in the queue. Strong confluence guarantees that the order of resolution is irrelevant (SPEC-01, T4; DISC-005 v2, Section 8.2; ARG-001, Step 4). **(MUST)**

**R19.** The number of interactions performed during border redex resolution SHOULD be recorded separately in `GridMetrics.border_interactions_per_round` to distinguish boundary work from local work. **(SHOULD)**
...
- `compute_time_per_round: Vec<Duration>` -- local reduction time per round.
- `merge_time_per_round: Vec<Duration>` -- merge + border resolution time per round.
...
**R37.** The `WorkerRoundStats` structure SHOULD contain:
- `worker_id: WorkerId`
...
- The redex queue contains both residual local redexes and newly detected border redexes.
        ...
        for p in 0..total_ports(agent.symbol):
            ...
            if target != DISCONNECTED:
                match target:
                    ...
                    FreePort(_):
                        // Boundary connection: will be restored in Step 3
                        // Temporarily mark as DISCONNECTED
                        result.set_port(AgentPort(agent.id, p), DISCONNECTED)
```

Note on local redex queues: residual redexes from partition queues are discarded during merge. Step 3 and the subsequent `reduce_all` will detect redexes via `connect`.
...
**Alternative:** Instead of draining, a full scan of the port array could detect redexes not present in the queue. This is more robust but more expensive: O(A * PORTS_PER_SLOT). The drain approach is sufficient because `connect` (SPEC-02, R13) guarantees that every created redex is inserted into the queue.
...
fn run_grid(net, config, strategy) -> (Net, GridMetrics):
    ...
    loop:
        ...
        if current_net.redex_queue.is_empty():
            // Full scan to confirm (discard stale entries)
        ...
        let (merged_net, border_redex_count) = merge(
            plan.partitions,
            &plan.borders,
        )
        ...
        // Resolve border redexes (and any residual redexes)
        ...
        metrics.border_interactions_per_round
            .push(border_interactions as u64)
        metrics.total_interactions +=
            local_interactions + border_interactions as u64
...
2. **Each round makes progress.** In each round:
   ...
   - If neither local redexes nor border redexes existed, the net was already in Normal Form and the loop terminates at the check at the beginning of the next iteration.
...
```
   ...
   +================+                                         |
           |                                                  |
           v                                                  |
   [1] redex_queue empty? ------YES-----> NORMAL FORM         |
           |                              (return result)     |
           NO                                                 |
           |                                                  |
           v                                                  |
   [2] max_rounds reached? ---YES-----> TIMEOUT               |
           |                            (return partial)      |
           NO                                                 |
           |                                                  |
           v                                                  |
   [3] SPLIT                                                  |
       ...
       -> PartitionPlan { partitions, borders }               |
           |                                                  |
           v                                                  |
   [4] LOCAL REDUCTION (per worker, in parallel)              |
       For each partition:                                    |
         reduce_all(partition.subnet)                         |
         rebuild_free_port_index(partition.subnet)            |
           |                                                  |
           v                                                  |
   [5] MERGE                                                  |
       merge(partitions, borders)                             |
       -> (merged Net, border_redex_count)                    |
           |                                                  |
           v                                                  |
   [6] RESOLVE BORDERS + RESIDUAL REDEXES                     |
       reduce_all(merged_net)                                 |
           |                                                  |
           v                                                  |
   [7] Accumulate metrics, rounds++                           |
           |                                                  |
           +--------------------------------------------------+
```
...
3. **Choice between drain_stale_redexes and full scan for Normal Form detection.** The `drain_stale_redexes` function (Section 4.4) is sufficient if `connect` never fails to insert a redex. If a bug causes a "lost redex" (a redex not inserted into the queue), only a full scan of the port array would detect it. The decision to include or not include the full scan as an additional verification is left to the ENGINEER and to SPEC-08 (test strategy).

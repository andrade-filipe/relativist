...
**Status:** Revised v3.1 — R6 and §4.3 amended per SPEC-22 §3.8 A6 (I3' debug-assertion language)
**Depends on:** SPEC-00 (Glossary), SPEC-01 (Invariants), SPEC-02 (Net Representation)
**Amends:** SPEC-22 §3.8 A6 (§4.3 debug-assertion allowlist/denylist for I3'-compatible assertions)
**Gray zones resolved:** ---
...
**R6.** Each rule MUST preserve invariant I3' (Uniqueness of AgentIds) from SPEC-01: `next_id` is strictly greater than any `AgentId` ever assigned, and no two live agents share the same `AgentId`. Rule implementations MUST NOT assume monotonicity of returned `AgentId` values across multiple `create_agent` calls in a single rule fire (see §4.3 amendment). **(MUST)**
...
- I3' (uniqueness): The 4 returned IDs are unique among live agents; `next_id` is incremented only for IDs NOT drawn from the free-list (SPEC-22 R3 / SPEC-01 I3'). The 4 IDs are NOT guaranteed to be monotonically increasing when the free-list is non-empty (see §4.3 amendment).
...
- I3' (uniqueness): 2 new IDs generated by `create_agent`; IDs are unique among live agents (SPEC-01 I3').
...
#### 4.3.0 Debug Assertion Language — I3'-Compatible Patterns (Amendment A6)

> **Amendment A6 (SPEC-22 §3.8 A6 / R27a):** Rule implementations MUST NOT assume monotonicity of returned IDs across multiple `create_agent` calls in a single rule fire. Allowed assertion patterns: `debug_assert!(self.agents[new_id as usize].is_some())` (uniqueness check post-create), `debug_assert!(self.next_id > new_id)` (next_id upper-bound check). Forbidden assertion patterns: `assert!(new_id > old_max_id)`, `assert!(new_id == self.next_id - 1)`, or any other monotonicity claim. The CON-DUP commutation rule is the load-bearing case: it creates 4 agents per fire and MAY receive any mix of recycled (smaller) and fresh (larger) IDs from the free-list. The 4 returned IDs are NOT guaranteed to form a contiguous monotonic block under SPEC-22. `assert_next_id_valid` (SPEC-02 §4.5) is preserved — its `(i as u32) < self.next_id` check for `slot.is_some()` is consistent with I3' (free-list IDs are in `None` slots, so they do not trip the assertion). The Stage 3 DEVELOPER audit (TASK-0497) MUST scan `src/reduction/` for forbidden patterns and reformulate them per this allowlist. Closes SC-010. See SPEC-22 R27a.

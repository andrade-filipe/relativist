...
- **Reproducao em outra maquina:** use `reproduce_article/scripts/reproduce_local_baseline.sh` (ver §5).
...
```bash
ls reproduce_article/results/locked/v1_local_baseline/
# Esperado: README.md manifest.md  (apenas templates)

# Se houver phase1_*.csv ou raw/ de runs anteriores, mova para backup:
# mv reproduce_article/results/locked/v1_local_baseline/phase1_* /tmp/old_snapshot/
```
...
bash reproduce_article/scripts/bench_phase1_locked.sh 2>&1 | tee -a /tmp/v1_baseline.log
...
awk -F, 'NR>1 && $6=="false"' reproduce_article/results/locked/v1_local_baseline/phase1_lenient_detail.csv | wc -l
awk -F, 'NR>1 && $6=="false"' reproduce_article/results/locked/v1_local_baseline/phase1_strict_detail.csv | wc -l
# Ambos DEVEM imprimir 0

# (b) Row counts
wc -l reproduce_article/results/locked/v1_local_baseline/phase1_lenient_detail.csv
wc -l reproduce_article/results/locked/v1_local_baseline/phase1_strict_detail.csv
wc -l reproduce_article/results/locked/v1_local_baseline/phase1_lenient_summary.csv
wc -l reproduce_article/results/locked/v1_local_baseline/phase1_strict_summary.csv

# (c) Todos os benchmarks presentes
awk -F, 'NR>1 {print $1}' reproduce_article/results/locked/v1_local_baseline/phase1_lenient_summary.csv | sort -u
...
bash reproduce_article/scripts/bench_phase2_locked.sh 2>&1 | tee -a /tmp/v1_baseline.log
...
# Zero correct=false
awk -F, 'NR>1 && $6=="false"' reproduce_article/results/locked/v1_local_baseline/phase2_detail.csv | wc -l  # = 0

# Contagens
wc -l reproduce_article/results/locked/v1_local_baseline/phase2_detail.csv
# Esperado: 329 (8 sequencial + 8*4*10 Docker + 1 header)
wc -l reproduce_article/results/locked/v1_local_baseline/phase2_summary.csv
# Esperado: 41

# Todas as 8 combinacoes bench*size
awk -F, 'NR>1 {print $1 "_" $2}' reproduce_article/results/locked/v1_local_baseline/phase2_summary.csv | sort -u | wc -l  # = 8
```
...
# Checksums
for f in reproduce_article/results/locked/v1_local_baseline/phase*_*.csv; do
    sha256sum "$f"
done

# Row counts
for f in reproduce_article/results/locked/v1_local_baseline/phase{1,2}_*.csv; do
    printf "%-60s %s\n" "$(basename "$f")" "$(wc -l < "$f")"
done
```
...
```bash
python3 reproduce_article/scripts/cv_triage.py
# Gera reproduce_article/results/locked/v1_local_baseline/cv_triage.md com rows CV > 0.15
...
```bash
cd codigo/relativist
git add reproduce_article/results/locked/v1_local_baseline/
...
cargo build --release
bash reproduce_article/scripts/reproduce_local_baseline.sh
```

O script roda Phase 1 + Phase 2 em `reproduce_article/results/reproduction/<data>/` e gera `comparison.md` contra a referencia. Wall-clock diverge (hardware diferente), mas colunas estruturais **devem casar exatamente**.
...
| manifest.md mal preenchido | Valores perdidos | `git checkout reproduce_article/results/locked/v1_local_baseline/manifest.md` recupera o template |

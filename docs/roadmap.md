...
**v2 empirical update (post-D-012, 2026-05-05).** The canonical v2 baseline `reproduce_article/results/locked/v2_post_d012_baseline_2026-05-05/` (32 distributed slots × 10 reps, all `all_correct=true`) shows v2 sends **30 % fewer bytes per round** (28.7 MB vs 41 MB on `ep_500k w=1`) with delta protocol active — confirming §2.26's structural saving. However, on TCP-localhost the v2 wall-clock is 1.4–2.0× v1: the bytes-saved-on-wire don't pay for the CPU cost of the abstraction layers (SPEC-17/18/19/21/22). Per-round decomposition (D-012 instrumentation): `wall ≈ 47 % network + 8 % compute + 6 % merge + 38 % framing/orchestration`. The break-even where v2 wins is at LAN bandwidth ≤ 156 MB/s (≈1.2 Gbps) — every real LAN/WAN qualifies. **Phase 3 LAN measurement is required to publish the break-even crossover empirically** (`docs/benchmarks/phase-3-lan.md` Axis 1 + Axis 2).

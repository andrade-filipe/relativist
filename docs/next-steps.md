...
**Test floor advanced:** 1784→**1798** default / 1828→**1842** zero-copy / 1775→**1789** streaming-no-recycle / **1740** release (was broken pre-D-012). v1 floor=690 inviolable. All cargo gates green on all 4 profiles.
...
- **TASK-0621** — `cfg(debug_assertions)` audit across the codebase (TASK-0617 fixed 4 instances; QA flagged that broader manifestations may still exist)

**Bench harness implication:** the v2 baseline at `results/locked/v2_d011_final_baseline_2026-05-04/` was generated by HEAD `b079cdc` (pre-D-012). It still reflects the broken instrumentation (RF-04/05/07 zeroes). The fix is in production now (`c439182`), so any **new** bench run from `v2-development` HEAD will produce non-zero `network_time_secs`, `compute_time_secs`, and `mips_mean` columns. The locked baseline is preserved as historical reference; do not expect it to retroactively gain populated columns.
